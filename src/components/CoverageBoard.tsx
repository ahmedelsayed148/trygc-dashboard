...
function CoverageBoardContent() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return filterByDateRange(matchingRecords, dateRange, (record) => record.date || record.doneAt || record.updatedAt || record.createdAt);
  }, [data.records, dateRange, deferredSearch, ownerFilter, statusFilter, taskFilter]);
    ...
    <div className="px-4 py-4 sm:px-6 sm:py-6">
      <div className="mx-auto max-w-screen-2xl space-y-6">
        ...
        <section className="rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 sm:p-6">
          ...
          <div className="mt-4">
            <DateRangeFilter label="Coverage Date Range" value={dateRange} onChange={setDateRange} />
          </div>
                ...
                <tbody className="divide-y divide-zinc-100 bg-white dark:divide-zinc-900 dark:bg-zinc-950">
                  {paginatedRecords.map((record) => (
                    <tr key={record.id} className="align-top">
                      ...
                      <td className="max-w-[320px] px-4 py-4 whitespace-normal break-words text-zinc-700 dark:text-zinc-200">{record.campaign || "—"}</td>
                      ...
                      <td className="max-w-[320px] px-4 py-4 whitespace-normal break-words text-zinc-600 dark:text-zinc-300">{[record.warning, record.action, record.notes].filter(Boolean).join(" | ") || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
