...
export function PersonalDashboard({ tasks, allTasks, userName, userEmail, onEditTask, successLogs }: PersonalDashboardProps) {
  ...
  const myStats = useMemo(() => {
    ...
    const todayCON = tasks.reduce((a, t) => a + (t.metricConfirmationToday || 0), 0);
  ...
  const myRank = useMemo(() => {
    const agents = [...new Set(allTasks.map(t => t.assignedTo))].filter(Boolean);
    const scores = agents.map(name => {
      const at = allTasks.filter(t => t.assignedTo === name);
      ...
      {activeView === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4">
            <QuickStat icon={Target} label="My Tasks" value={myStats.total} color="bg-black dark:bg-white dark:text-black" />
            <QuickStat icon={CheckCircle2} label="Completed" value={myStats.done} color="bg-zinc-700 dark:bg-zinc-300 dark:text-black" />
            <QuickStat icon={Clock} label="In Progress" value={myStats.inProgress} color="bg-zinc-600 dark:bg-zinc-400 dark:text-black" />
            <QuickStat icon={Zap} label="Today CON" value={myStats.todayCON} color="bg-zinc-500" />
            <QuickStat icon={Shield} label="SLA Rate" value={`${myStats.slaRate}%`} color="bg-zinc-800 dark:bg-zinc-200 dark:text-black" />
            <QuickStat icon={Flame} label="Overdue" value={myStats.overdue} color="bg-zinc-900 dark:bg-zinc-100 dark:text-black" />
          </div>
...
function QuickStat({ icon: Icon, label, value, color }: { icon: any; label: string; value: any; color: string }) {
  return (
    <div className="bg-white dark:bg-zinc-950 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
      <div className={`${color} w-9 h-9 rounded-xl flex items-center justify-center text-white shadow-md mb-3`}>
