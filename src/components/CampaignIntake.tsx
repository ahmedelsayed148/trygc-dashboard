...
function CampaignIntakeContent() {
  ...
  const filteredCampaignIntakes = useMemo(
    () => filterByDateRange(campaignIntakes, dateRange, (item) => item.startDate || item.createdAt),
    [campaignIntakes, dateRange],
  );

  const stats = useMemo(() => {
    const upcoming = filteredCampaignIntakes.filter((item) => item.stage === "Upcoming").length;
    const ongoing = filteredCampaignIntakes.filter((item) => item.stage === "Ongoing").length;
    const synced = filteredCampaignIntakes.filter((item) => item.linkedCampaignId).length;
  ...
  }, [filteredCampaignIntakes, opsCampaigns.length]);
  ...
  return (
    <div className="min-h-screen bg-zinc-50 px-4 py-4 sm:px-6 sm:py-6 dark:bg-black">
      <div className="mx-auto max-w-screen-2xl space-y-6">
        ...
        <DateRangeFilter label="Intake Date Range" value={dateRange} onChange={setDateRange} />
        ...
        <section className="rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
            {filteredCampaignIntakes.length > 0 ? (
              filteredCampaignIntakes.slice(0, 8).map((item) => (
                <article
                  key={item.id}
