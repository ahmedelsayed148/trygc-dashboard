...
import { playWorkspaceAlertSound } from '../lib/realtimeAlerts';
import { requestDesktopNotificationPermission } from '../lib/notificationPreferences';
...
export function Settings() {
  ...
  const notificationItems: Array<{
  ...
  }> = [
    {
      ...
      desc: 'Show instant alerts when work is assigned to you.',
      value: preferences.taskAssignments,
    },
    {
      key: 'successLogs',
      title: 'Team Updates',
      desc: 'Show live alerts when new workspace updates are posted.',
      value: preferences.successLogs,
    },
    {
      key: 'soundAlerts',
      title: 'Sound Alerts',
      desc: 'Play a short sound when a realtime alert arrives.',
      value: preferences.soundAlerts,
    },
    {
      key: 'desktopAlerts',
      title: 'Desktop Alerts',
      desc: 'Send browser notifications when permission is granted.',
            ...
            {activeSection === 'notifications' && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-zinc-600 dark:text-zinc-400">Realtime alerts for assignments and team updates</p>
                </div>
                <div className="space-y-4">
                  {notificationItems.map(item => (
                    <div key={item.title} className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-900 rounded-2xl">
                      ...
                      <button
                        type="button"
                        onClick={() => updatePreferences((current) => ({ ...current, [item.key]: !current[item.key] }))}
                        className={cn(
                          'inline-flex h-7 w-12 items-center rounded-full p-1 transition-colors',
                          item.value ? 'bg-black dark:bg-white' : 'bg-zinc-200 dark:bg-zinc-700',
                        )}
                        aria-pressed={item.value}
                      >
                        <span
                          className={cn(
                            'h-5 w-5 rounded-full bg-white transition-transform dark:bg-zinc-900',
                            item.value && 'translate-x-5 bg-zinc-100 dark:bg-zinc-950',
                          )}
                        />
                      </button>
                    </div>
                  ...
                  <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div>
                        <div className="font-bold text-zinc-800 dark:text-zinc-100">Browser permission</div>
                        <div className="text-sm text-zinc-500 dark:text-zinc-400">
                          Current state: <span className="font-semibold capitalize">{notificationPermission}</span>
                        </div>
                      </div>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            if (permission !== 'granted') {
                              updatePreferences((current) => ({ ...current, desktopAlerts: false }));
                              return;
                            }
                            updatePreferences((current) => ({ ...current, desktopAlerts: true }));
                          }}
                          className="rounded-xl border border-zinc-200 px-4 py-2 text-sm font-bold text-zinc-700 transition-colors hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-900"
                        >
                          Enable desktop alerts
                        </button>
                        <button
                          type="button"
                          onClick={() => void playWorkspaceAlertSound()}
                          className="app-accent-button rounded-xl px-4 py-2 text-sm font-bold"
                        >
                          Test sound
                        </button>
                      </div>
