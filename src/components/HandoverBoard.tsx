...
function HandoverBoardContent() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      critical: savedHandovers.filter((item) => item.health === "Critical").length,
      watch: savedHandovers.filter((item) => item.health === "Watch").length,
      today: savedHandovers.filter(
        ...
        <section className="rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 sm:p-6">
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
            {savedHandovers.length > 0 ? (
              savedHandovers.slice(0, 8).map((item) => (
                <article
                  key={item.id}
