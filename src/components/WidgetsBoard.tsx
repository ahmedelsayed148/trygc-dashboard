...
function WidgetsBoardContent() {
  ...
  }, [linkWidgets, search, selectedCategory]);
            ...
            {filteredWidgets.length > 0 ? (
              filteredWidgets.map((widget) => {
                ...
                return (
                  <article
                    key={widget.id}
                    className={`rounded-[1.75rem] border ${palette.ring} bg-zinc-50 p-4 shadow-sm transition-all dark:bg-zinc-900 sm:p-5 ${
                      ...
                      <div className="flex min-w-0 items-start gap-4">
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            {widget.pinned && <Pin className="h-4 w-4 text-zinc-500" />}
                            <h3 className="truncate text-lg font-black text-zinc-900 dark:text-zinc-100 sm:text-xl">
                              {widget.title}
                            </h3>
                            {widget.category && (
                              <span className="rounded-full bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-zinc-500 dark:bg-zinc-950 dark:text-zinc-300">
                                {widget.category}
                              </span>
                            )}
                          </div>
                          <p className="mt-2 break-all text-sm text-zinc-500">{widget.url}</p>
