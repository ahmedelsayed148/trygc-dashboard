...
export function createCampaignTask(teamId: string, seed?: Partial<CampaignTeamTask>): CampaignTeamTask {
  ...
  if (seed?.metricConfirmationToday !== undefined) nextTask.metricConfirmationToday = Number(seed.metricConfirmationToday) || 0;
  if (seed?.metricMissingDate !== undefined) nextTask.metricMissingDate = Number(seed.metricMissingDate) || 0;
  if (seed?.metricWithDate !== undefined) nextTask.metricWithDate = Number(seed.metricWithDate) || 0;
  if (seed?.resultSummary !== undefined) nextTask.resultSummary = seed.resultSummary;
  if (seed?.blocker !== undefined) nextTask.blocker = seed.blocker;
  if (seed?.nextStep !== undefined) nextTask.nextStep = seed.nextStep;
  if (seed?.finalOutput !== undefined) nextTask.finalOutput = seed.finalOutput;
